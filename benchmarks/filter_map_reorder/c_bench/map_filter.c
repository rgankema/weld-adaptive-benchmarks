...
void work(uint64_t N, vector_t * __restrict out, tuple_t * __restrict tmp, 
    ...
    // tuple_t * tmp = malloc(sizeof(tuple_t) * N);
    for (int64_t i = 0; i < N; i++) {
        ...
        int32_t e4 = in5[i];
        int32_t e5 = in6[i];
        tmp[i].a = e0 * e1 * e2 * e3 * e4;
        tmp[i].b = e1 * e2 * e3 * e4 * e5;
        tmp[i].c = e0 * e5 + e1 * e4;
    }
    for (int64_t i = 0; i < N; i++) {
        if (in1[i] == 42) {
...
int32_t main(int argc, char** argv) {
    ...
    tuple_t *tmp = (tuple_t*)malloc(sizeof(tuple_t) * N);
    uint64_t res = 0;
    ...
    for (int32_t i = 0; i < iters; i++) {
        ...
        work(N, out, tmp, in1, in2, in3, in4, in5, in6);
